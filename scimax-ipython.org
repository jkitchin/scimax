...
#+BEGIN_SRC sh
cat `jupyter --runtime-dir`/kernel-223460.json
#+END_SRC
...
| hb_port          | 34935,                            |
| ip               | 127.0.0.1                         |
| key              | 044c52eb-e49cd8e078a283a224352f9b |
| transport        | tcp                               |
...
#+BEGIN_SRC sh
jupyter console --existing kernel-223460.json kitchin@localhost
#+END_SRC

This opens a local Ipython terminal. In that terminal, I typed a = 5.

Then, you specify the kernel connection file as the :session argument in an ipython block.

#+BEGIN_SRC ipython :session kernel-223460.json
print(a)
...
You can see from  the output that we successfully connected to the kernel and that indeed a=5. Furthermore, in the block above we assigned b=6, and in the terminal I can type "b" and see that it is equal to 6.
