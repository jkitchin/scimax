...
;; (setq custom-file (expand-file-name "user/custom.el" load-dir))
;; (when (f-exists? custom-file) (load custom-file))

(setq auto-save-list-file-prefix (expand-file-name "auto-save-list/saves-" load-dir))

;; abbrevs
;; (setq abbrev-file-name (expand-file-name "user/abbrev_defs" load-dir))
(setq save-abbrevs t)
...
(defun scimax-update ()
  ...
  (interactive)
  (let ((default-directory load-dir))
...
(defun load-dired-cycle-space-hyphen-underscore ()
...
(define-key dired-mode-map (kbd "-") 'load-dired-cycle-space-hyphen-underscore)
