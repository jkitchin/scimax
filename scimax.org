...
Spell-checking is configured with ivy, so if the word is correct, you can type M-o s to save the word in your dictionary so you will not see it again
...
If you install the Matlab kernel: https://github.com/Calysto/matlab_kernel, it is possible to run Matlab from Emacs in a moderately reasonable way, e.g. in a session.

You need to add these lines to your init files.
...
It is not a very rich interface. You have to format some things yourself to get tables, and there are no inline images.
