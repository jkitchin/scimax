...
It is hard to believe it is over two years again since the last big update. I have some renewed energy in scimax with the development of LLMs, and integration of them in Emacs with packages like gptel.
...
- rewrite of org-db to enable semantic search
...
It appears that AUCTeX may not be automatically installed on Linux ([[https://github.com/jkitchin/scimax/issues/498][issue]]) when using the archived installation method. I don't know if this is an issue with homebrew or not.
