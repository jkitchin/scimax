...
default name based on the url is suggested."
  (interactive (list (read-input "git url: ") nil))
...
Here we define a new org-link for making links to files in notebooks. These links will look like nb:project-name::relative-file-path::link-target
  ...
  (let* ((link (org-element-context))
	...
	(let* ((projects (remove nil (append (projectile-relevant-known-projects)
	  ...
	  (if (= 1 (length candidates))
	    ...
	    (bash (read-input "Project: " candidates))))))))
  ...
  (let* ((link (org-element-context))
	...
	(let* ((projects (remove nil (append (projectile-relevant-known-projects)
	  ...
	  (if (= 1 (length candidates))
	    ...
	    (explorer (read-input "Project: " candidates))))))))
...
(defvar nb-link-map (let ((map (copy-keymap org-mouse-map)))
		      ...
		      (define-key map (kbd "M-f") 'nb-link-explorer)
		      map)
