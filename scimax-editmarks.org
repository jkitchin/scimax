...
(defun sem-export-insert (backend)
  ...
  (let ((bounds (sem-editmark-bounds))
    ...
    (cond
     ...
     ((eq 'html backend)
      (setf (buffer-substring (car bounds) (cdr bounds))
	    (format "@@html:<p style=\"color:blue\">%s</p>@@"
		    (buffer-substring-no-properties (car content-bounds)
						    (cdr content-bounds))))))))
...
(defun sem-export-delete (backend)
  ...
  (let ((bounds (sem-editmark-bounds))
    ...
    (cond
     ...
     ((eq 'html backend)
      (setf (buffer-substring (car bounds) (cdr bounds))
	    (format "@@html:<p style=\"color: red\">%s</p>@@"
		    (buffer-substring-no-properties (car content-bounds)
						    (cdr content-bounds))))))))
...
(defun sem-export-comment (backend)
  ...
  (let ((bounds (sem-editmark-bounds))
    ...
    (cond
     ...
     ((eq 'html backend)
      (setf (buffer-substring (car bounds) (cdr bounds))
	    (format "@@html:<p style=\"color: orange\">%s</p>@@"
		    (buffer-substring-no-properties (car content-bounds)
						    (cdr content-bounds))))))))
...
(defun sem-export-task (backend)
  ...
  (let ((bounds (sem-editmark-bounds))
    ...
    (cond
     ...
     ((eq 'html backend)
      (setf (buffer-substring (car bounds) (cdr bounds))
	    (format "@@html:<p style=\"color: purple\">%s</p>@@"
		    (buffer-substring-no-properties (car content-bounds)
						    (cdr content-bounds))))))))
#+END_SRC
...
we go with the font color."
  (let* ((bounds (sem-editmark-bounds))
    ...
    (cond
     ...
     ((eq 'html backend)
      (setf (buffer-substring (car bounds) (cdr bounds))
	    (format "@@html:<p style=\"color: %s; background-color: %s\">%s</p>@@"
		    fg-hex bg-hex
		    (buffer-substring-no-properties
		     (car content-bounds)
		     (cdr content-bounds))))))))
...
*************** TODO Can thingatpt help here?
It seems to use forward/backward motion to define the bounds.
*************** END
...
*** LaTeX
 ...
 It is convenient to convert editmarks into LaTeX code so that we can convert them to PDFs. Each editmark should define an export function that takes a single argument, the backend, that is responsible for converting the editmark to the export syntax.

 #+BEGIN_SRC emacs-lisp
(defun sem-editmarks-to-org-latex (&optional backend)
  "Convert sem editmarks in an org-file to LaTeX.
...
\\listoftodos\n")
  ...
  (while (sem-next-editmark)
    (let ((export-func (plist-get (cdr (assoc (get-text-property (point) 'sem-type) sem-editmarks)) :export)))
      (if export-func
	  (funcall export-func 'latex)
	(sem-export-default 'latex)))))

 #+END_SRC

 #+RESULTS:
 : sem-editmarks-to-org-latex

 To /conveniently/ get to a pdf we do some gymnastics. I thought that using a preprocessing hook would work, but it seems the markup gets escaped pretty early, so instead we save the buffer content, modify the buffer, export it, and put the buffer back the way it was. This function makes a pdf for you.

 #+BEGIN_SRC emacs-lisp
(defun sem-editmarks-to-pdf ()
  "Convert current buffer to a PDF with editmarks."
  (interactive)
  (when sem-track-change-mode (error "You cannot make a pdf in track change mode."))
  (atomic-change-group
    (let ((org (buffer-string)))
      ;; Replace editmarks, then export to pdf
      (sem-editmarks-to-org-latex)
      (org-open-file (org-latex-export-to-pdf))
      ;; Reset the buffer to original content.
      (setf (buffer-string) org))))


 #+END_SRC

 #+RESULTS:
 : sem-editmarks-to-pdf


*** HTML

 #+BEGIN_SRC emacs-lisp
(defun sem-editmarks-to-org-html (&optional backend)
  "Convert sem editmarks in an org-file to LaTeX.
Inserts some headers at the top for todonotes and ulem, and the
LaTeX markup commands. This is not super robust, but works for
simple changes. There are issues with changes in citations,
...
in a copy of the buffer."
  ...
  (while (sem-next-editmark)
    (let ((export-func (plist-get (cdr (assoc (get-text-property (point) 'sem-type) sem-editmarks)) :export)))
      (if export-func
	  (funcall export-func 'html)
	(sem-export-default 'html)))))


(defun sem-editmarks-to-html ()
  "Convert current buffer to a PDF with editmarks."
  (interactive)
  (when sem-track-change-mode (error "You cannot make an html in track change mode."))
  (atomic-change-group
    (let ((org (buffer-string)))
      ;; Replace editmarks, then export to pdf
      (sem-editmarks-to-org-html)
