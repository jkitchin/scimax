...
1. It points to a location, and if you change the file, the location will not be the same as it was when you saved the link.
...
This way it is not necessary to put quotes on the values, so the plist above will end up as:
 (:keyword \"some values\" :next-keyword \"more values\")

Then, this gets read by elisp to make the plist. The downside of this is it is not feasible to have a key without a value."
...
Maybe an md5 hash too, so you can tell if the file has changed since the link was made.
...
TYPE should be a symbol corresponding to the car of an entry in `sem-editmarks'."
  ...
  ;; reliable.
  (if (not (member '(eval sem-mode) file-local-variables-alist))
      (let ((mode major-mode))
	(save-excursion
	  (save-restriction
	    (widen)
